...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    for label, im_data in visuals.items():
        ...
        for ind in range(batch_size):
            ...
            im = util.tensor2im(im_data[ind])
            i_short_path = ntpath.basename(image_path[ind])
            i_name = os.path.splitext(i_short_path)[0]
            image_name = '%s_%s.png' % (i_name, label)
...
class Visualizer():
    ...
    def print_current_val(self, epoch, iters, message):
        print(message)
        if util.is_main_process():
            with open(self.log_name, "a") as log_file:
                log_file.write('%s\n' % message)  # save the message
