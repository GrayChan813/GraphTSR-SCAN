...
class Cell_Lloc_Pre(nn.Module):
    def __init__(self, num_node_feature, vocab_size, num_text_features, num_classes,
        ...
        # 空间特征分支
        self.gcn_pre = nn.Sequential(nn.Linear(num_node_feature, 256), nn.ReLU(inplace=True),
        ...
        self.gcn_conv1 = GCNConv(512, 512)
        self.gcn_norm1 = norm.LayerNorm(512)
        self.gcn_conv2 = GCNConv(512, 512)
        self.gcn_norm2 = norm.LayerNorm(512)
        self.gat_conv1 = GATConv(512, 512)
        self.gat_norm1 = norm.LayerNorm(512)
        self.gat_conv2 = GATConv(512, 512)
        self.gat_norm2 = norm.LayerNorm(512)

        # self.gcn_post_row = nn.Sequential(nn.Linear(512, 256), nn.ReLU(inplace=True),
        #                              nn.Linear(256, 256), nn.ReLU(inplace=True))
        # self.gcn_post_col = nn.Sequential(nn.Linear(512, 256), nn.ReLU(inplace=True),
        #                              nn.Linear(256, 256), nn.ReLU(inplace=True))
        self.gcn_post = nn.Sequential(nn.Linear(512, 256), nn.ReLU(inplace=True),
                                     nn.Linear(256, 256), nn.ReLU(inplace=True))
        ...
        #                                       nn.Linear(256, 256), nn.ReLU(inplace=True))
        # self.gcn_post_img = nn.Sequential(nn.Linear(512, 256), nn.ReLU(inplace=True), nn.Dropout(),
        #                                   nn.Linear(256, 256), nn.ReLU(inplace=True))

        # 文本特征分支
        # self.text_embeds = nn.Embedding(vocab_size, num_text_features)
        self.rnn_text = nn.GRU(num_text_features, 256, bidirectional=False, batch_first=True)
        # self.gcn_pre_text = nn.Sequential(nn.Linear(num_text_features, 512), nn.ReLU(inplace=True), nn.Dropout(),
        #                              nn.Linear(512, 256), nn.ReLU(inplace=True))
        ...
        #                                    nn.Linear(256, 256), nn.ReLU(inplace=True))
        # self.gcn_post_text = nn.Sequential(nn.Linear(512, 256), nn.ReLU(inplace=True), nn.Dropout(),
        #                                    nn.Linear(256, 256), nn.ReLU(inplace=True))

        # 特征融合分支
        # self.fc_fusion_row = nn.Sequential(*[nn.Linear(256 * 3, 512), nn.ReLU(inplace=True),
        #                                    nn.Linear(512, 256), nn.ReLU(inplace=True)])
        # self.fc_fusion_col = nn.Sequential(*[nn.Linear(256 * 3, 512), nn.ReLU(inplace=True),
        #                                    nn.Linear(512, 256), nn.ReLU(inplace=True)])
        self.fc_fusion_row = nn.Sequential(*[nn.Linear(256 * 3, 128), nn.ReLU(inplace=True)])
        self.fc_fusion_col = nn.Sequential(*[nn.Linear(256 * 3, 128), nn.ReLU(inplace=True)])
        ...
        #                                   nn.Linear(256, num_classes), nn.ReLU(inplace=True)])
        self.fc_row_cls = nn.Sequential(*[nn.Linear(128 * 2, num_classes), nn.ReLU(inplace=True)])
        self.fc_col_cls = nn.Sequential(*[nn.Linear(128 * 2, num_classes), nn.ReLU(inplace=True)])
    ...
    def forward(self, x, img, imgpos, text, edge_index, edge_pairing_index, phase):
        ...
        box_feat_gcn = self.gcn_conv1(box_feat, edge_index)
        box_feat_gcn = self.gcn_norm1(box_feat_gcn)
        box_feat_gcn = F.relu(box_feat_gcn)
        box_feat_gat = self.gat_conv1(box_feat, edge_index)
        box_feat_gcn = self.gat_norm1(box_feat_gcn)
        box_feat_gat = F.relu(box_feat_gat)
        box_feat = self.w[0] * box_feat_gcn + (1 - self.w[0]) * box_feat_gat

        box_feat_gcn = self.gcn_conv2(box_feat, edge_index)
        box_feat_gcn = self.gcn_norm2(box_feat_gcn)
        box_feat_gcn = F.relu(box_feat_gcn)
        box_feat_gat = self.gat_conv2(box_feat, edge_index)
        box_feat_gcn = self.gat_norm2(box_feat_gcn)
        box_feat_gat = F.relu(box_feat_gat)
        box_feat = self.w[1] * box_feat_gcn + (1 - self.w[1]) * box_feat_gat

        # box_feat_row = self.gcn_post_row(box_feat)
        # box_feat_col = self.gcn_post_col(box_feat)
        box_feat = self.gcn_post(box_feat)
        ...
        # img_feat = self.gcn_post_img(img_feat)
    
        # 文本特征
        # text_feat = self.text_embeds(text)
        text_feat = F.relu(text)
        text_feat, _ = self.rnn_text(text_feat)
        text_feat = text_feat[:, -1, :]
        # text_feat = self.gcn_pre_text(text_feat)
        ...
        # text_feat = self.gcn_post_text(text_feat)


        fusion_feat_row = torch.cat((box_feat, img_feat, text_feat), 1)
        fusion_feat_col = torch.cat((box_feat, img_feat, text_feat), 1)
