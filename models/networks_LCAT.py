...
class Cell_Lloc_Pre(nn.Module):
    def __init__(self, num_node_feature, vocab_size, num_text_features, num_classes,
        ...
        # 空间特征分支
        self.gnn_pre_bbox = nn.Sequential(nn.Linear(num_node_feature, 256), nn.PReLU(), nn.Dropout(),
                                     nn.Linear(256, 512), nn.PReLU())

        # self.gcn_conv1_bbox = Sequential('x, x_0, edge_index',
        #                             [(GCN2Conv(512, 512), 'x, x_0, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])
        # self.gcn_conv2_bbox = Sequential('x, x_0, edge_index',
        #                             [(GCN2Conv(512, 512), 'x, x_0, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])
        self.gcn_conv1_bbox = Sequential('x, edge_index',
                                    [(GCNConv(512, 512), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])
        self.gcn_conv2_bbox = Sequential('x, edge_index',
                                    [(GCNConv(512 , 512), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])
        self.gcn_conv3_bbox = Sequential('x, edge_index',
                                    [(GCNConv(512 * 8, 512), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])
        self.gat_conv1_bbox = Sequential('x, edge_index',
                                    [(GATv2Conv(512, 512, heads=1), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])
        self.gat_conv2_bbox = Sequential('x, edge_index',
                                    [(GATv2Conv(512, 512, heads=1), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])
        self.gat_conv3_bbox = Sequential('x, edge_index',
                                    [(GATv2Conv(512 * 8, 512, heads=1), 'x, edge_index -> x'), norm.LayerNorm(512),nn.PReLU(), nn.Dropout(), ])

        self.gnn_post_bbox_row = nn.Sequential(nn.Linear(512, 256), nn.PReLU(), nn.Dropout(),
                                     nn.Linear(256, 256), nn.PReLU())
        self.gnn_post_bbox_col = nn.Sequential(nn.Linear(512, 256), nn.PReLU(), nn.Dropout(),
                                     nn.Linear(256, 256), nn.PReLU())
        # self.gnn_post_bbox = nn.Sequential(nn.Linear(512, 512), nn.ReLU(inplace=True), nn.Dropout(),
        ...
        self.cnn_emb = nn.Sequential(
                        nn.Linear(64*7*7, 1024), nn.PReLU(), nn.Dropout(),
                        nn.Linear(1024, 512), nn.PReLU(), nn.Dropout(),
                        nn.Linear(512, 512), nn.PReLU())
        # self.gcn_conv1_img = Sequential('x, x_0, edge_index',
        #                             [(GCN2Conv(512, 512), 'x, x_0, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])
        # self.gcn_conv2_img = Sequential('x, x_0, edge_index',
        #                             [(GCN2Conv(512, 512), 'x, x_0, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])
        ...
        self.gat_conv2_img = Sequential('x, edge_index',
                                    [(GATv2Conv(512, 512, heads=1), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])
        self.gnn_post_img_row = nn.Sequential(nn.Linear(512, 256), nn.PReLU(), nn.Dropout(),
                                              nn.Linear(256, 256), nn.PReLU())
        self.gnn_post_img_col = nn.Sequential(nn.Linear(512, 256), nn.PReLU(), nn.Dropout(),
                                              nn.Linear(256, 256), nn.PReLU())
        # self.gnn_post_img = nn.Sequential(nn.Linear(512, 512), nn.ReLU(inplace=True), nn.Dropout(),
        #                                       nn.Linear(512, 256), nn.ReLU(inplace=True))

        # 文本特征分支
        # self.text_embeds = nn.Embedding(vocab_size, num_text_features)
        self.pprelu1 = nn.PReLU()
        self.rnn_text = nn.GRU(num_text_features, 512, bidirectional=False, batch_first=True)
        self.pprelu2 = nn.PReLU()
        # self.gnn_pre_text = nn.Sequential(nn.Linear(512, 512), nn.ReLU(inplace=True), nn.Dropout(),
        ...
        self.gat_conv2_text = Sequential('x, edge_index',
                                    [(GATv2Conv(512, 512, heads=1), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])
        self.gnn_post_text_row = nn.Sequential(nn.Linear(512, 256), nn.PReLU(), nn.Dropout(),
                                           nn.Linear(256, 256), nn.PReLU())
        self.gnn_post_text_col = nn.Sequential(nn.Linear(512, 256), nn.PReLU(), nn.Dropout(),
                                           nn.Linear(256, 256), nn.PReLU())
        # self.gnn_post_text = nn.Sequential(nn.Linear(512, 512), nn.ReLU(inplace=True), nn.Dropout(),
        #                                        nn.Linear(512, 256), nn.ReLU(inplace=True))

        # 特征融合分支
        self.gnn_pre_row = nn.Sequential(nn.Linear(256 * 3, 512), nn.PReLU(), nn.Dropout(),
                                          nn.Linear(512, 512), nn.PReLU())
        self.gcn_conv1_row = Sequential('x, edge_index',
                                    [(GCNConv(512, 512), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(),])
        self.gcn_conv2_row = Sequential('x, edge_index',
                                    [(GCNConv(512, 512), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(),])
        self.gat_conv1_row = Sequential('x, edge_index',
                                    [(GATv2Conv(512, 512, heads=1), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])
        self.gat_conv2_row = Sequential('x, edge_index',
                                    [(GATv2Conv(512, 512, heads=1), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])

        self.gnn_pre_col = nn.Sequential(nn.Linear(256 * 3, 512), nn.PReLU(), nn.Dropout(),
                                         nn.Linear(512, 512), nn.PReLU())
        self.gcn_conv1_col = Sequential('x, edge_index',
                                    [(GCNConv(512, 512), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(),])
        self.gcn_conv2_col = Sequential('x, edge_index',
                                    [(GCNConv(512, 512), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(),])
        self.gat_conv1_col = Sequential('x, edge_index',
                                    [(GATv2Conv(512, 512, heads=1), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])
        self.gat_conv2_col = Sequential('x, edge_index',
                                    [(GATv2Conv(512, 512, heads=1), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), nn.Dropout(), ])

        self.gnn_post_row = nn.Sequential(*[nn.Linear(512, 512), nn.PReLU(), nn.Dropout(),
                                           nn.Linear(512, 512), nn.PReLU()])
        self.gnn_post_col = nn.Sequential(*[nn.Linear(512, 512), nn.PReLU(), nn.Dropout(),
                                           nn.Linear(512, 512), nn.PReLU()])

        self.fc_row_cls = nn.Sequential(*[nn.Linear(512 * 2, 512), nn.PReLU(), nn.Dropout(),
                                          nn.Linear(512, 256), nn.PReLU(), nn.Dropout(),
                                          nn.Linear(256, num_classes), nn.PReLU()])
        self.fc_col_cls = nn.Sequential(*[nn.Linear(512 * 2, 512), nn.PReLU(), nn.Dropout(),
                                          nn.Linear(512, 256), nn.PReLU(), nn.Dropout(),
                                          nn.Linear(256, num_classes), nn.PReLU()])
        ...
        # self.w_row = nn.Parameter(torch.Tensor([0.5, 0.5]))
        # self.w_col = nn.Parameter(torch.Tensor([0.5, 0.5]))
        self.w_bbox = nn.Parameter(torch.Tensor([0.5, 0.5, 0.5, 0.5]))
        self.w_img = nn.Parameter(torch.Tensor([0.5, 0.5, 0.5, 0.5]))
        self.w_text = nn.Parameter(torch.Tensor([0.5, 0.5, 0.5, 0.5]))
        self.w_row = nn.Parameter(torch.Tensor([0.5, 0.5, 0.5, 0.5]))
        self.w_col = nn.Parameter(torch.Tensor([0.5, 0.5, 0.5, 0.5]))
        self.alpha = nn.Parameter(torch.Tensor([1, 1]))
        self.beta = nn.Parameter(torch.Tensor([1, 1]))
        self.gamma = nn.Parameter(torch.Tensor([1, 1]))

    def forward(self, x, img, imgpos, text, edge_index, edge_pairing_index, phase):

        # print(self.w_bbox, self.w_img, self.w_text, self.w_row, self.w_col)
        # print(self.alpha, self.beta, self.gamma)
        _, _, self.img_h, self.img_w = img.shape

        # 几何特征bbox
        box_feat = self.gnn_pre_bbox(x)

        box_feat_gcn = self.gcn_conv1_bbox(box_feat, edge_index)
        box_feat = (1 - self.w_bbox[0]) * box_feat + self.w_bbox[0] * box_feat_gcn
        box_feat_gat = self.gat_conv1_bbox(box_feat, edge_index)
        box_feat = (1 - self.w_bbox[1]) * box_feat + self.w_bbox[1] * box_feat_gat
        
        box_feat_gcn = self.gcn_conv2_bbox(box_feat, edge_index)
        box_feat = (1 - self.w_bbox[2]) * box_feat + self.w_bbox[2] * box_feat_gcn
        box_feat_gat = self.gat_conv2_bbox(box_feat, edge_index)
        box_feat = (1 - self.w_bbox[3]) * box_feat + self.w_bbox[3] * box_feat_gat

        box_feat_row = self.gnn_post_bbox_row(box_feat)
        box_feat_col = self.gnn_post_bbox_col(box_feat)
        # box_feat = self.gnn_post_bbox(box_feat)
        ...
        img_feat = (1 - self.w_img[0]) * img_feat + self.w_img[0] * img_feat_gcn
        img_feat_gat = self.gat_conv1_img(img_feat, edge_index)
        img_feat = (1 - self.w_img[1]) * img_feat + self.w_img[1] * img_feat_gat
        
        img_feat_gcn = self.gcn_conv2_img(img_feat, edge_index)
        img_feat = (1 - self.w_img[2]) * img_feat + self.w_img[2] * img_feat_gcn
        img_feat_gat = self.gat_conv2_img(img_feat, edge_index)
        img_feat = (1 - self.w_img[3]) * img_feat + self.w_img[3] * img_feat_gat

        img_feat_row = self.gnn_post_img_row(img_feat)
        img_feat_col = self.gnn_post_img_col(img_feat)
        # img_feat = self.gnn_post_img(img_feat)
    
        # 文本特征
        # text_feat = self.text_embeds(text)
        text_feat = self.pprelu1(text)
        text_feat, _ = self.rnn_text(text_feat)
        text_feat = text_feat[:, -1, :]
        text_feat = self.pprelu2(text_feat)
        # text_feat = self.gnn_pre_text(text_feat)

        text_feat_gcn = self.gcn_conv1_text(text_feat, edge_index)
        text_feat = (1 - self.w_text[0]) * text_feat + self.w_text[0] * text_feat_gcn
        text_feat_gat = self.gat_conv1_text(text_feat, edge_index)
        text_feat = (1 - self.w_text[1]) * text_feat + self.w_text[1] * text_feat_gat
        
        text_feat_gcn = self.gcn_conv2_text(text_feat, edge_index)
        text_feat = (1 - self.w_text[2]) * text_feat + self.w_text[2] * text_feat_gcn
        text_feat_gat = self.gat_conv2_text(text_feat, edge_index)
        text_feat = (1 - self.w_text[3]) * text_feat + self.w_text[3] * text_feat_gat

        text_feat_row = self.gnn_post_text_row(text_feat)
        text_feat_col = self.gnn_post_text_col(text_feat)
        # text_feat = self.gnn_post_text(text_feat)

        # 融合特征
        box_feat_row, img_feat_row, text_feat_row = self.alpha[0] * box_feat_row, self.beta[0] * img_feat_row, self.gamma[0] * text_feat_row
        box_feat_col, img_feat_col, text_feat_col = self.alpha[1] * box_feat_col, self.beta[1] * img_feat_col, self.gamma[1] * text_feat_col

        # box_feat_row, img_feat_row = self.alpha[0] * box_feat_row, self.beta[0] * img_feat_row
        # box_feat_col, img_feat_col = self.alpha[1] * box_feat_col, self.beta[1] * img_feat_col
        ...
        fusion_feat_row = torch.cat((box_feat_row, img_feat_row, text_feat_row), 1)
        fusion_feat_row = self.gnn_pre_row(fusion_feat_row)
        fusion_feat_row_gcn = self.gcn_conv1_row(fusion_feat_row, edge_index)
        fusion_feat_row = (1 - self.w_row[0]) * fusion_feat_row + self.w_row[0] * fusion_feat_row_gcn
        fusion_feat_row_gat = self.gat_conv1_row(fusion_feat_row, edge_index)
        fusion_feat_row = (1 - self.w_row[1]) * fusion_feat_row + self.w_row[1] * fusion_feat_row_gat

        fusion_feat_row_gcn = self.gcn_conv2_row(fusion_feat_row, edge_index)
        fusion_feat_row = (1 - self.w_row[2]) * fusion_feat_row + self.w_row[2] * fusion_feat_row_gcn
        fusion_feat_row_gat = self.gat_conv2_row(fusion_feat_row, edge_index)
        fusion_feat_row = (1 - self.w_row[3]) * fusion_feat_row + self.w_row[3] * fusion_feat_row_gat
        fusion_feat_row = self.gnn_post_row(fusion_feat_row)


        fusion_feat_col = torch.cat((box_feat_col, img_feat_col, text_feat_col), 1)
        fusion_feat_col = self.gnn_pre_col(fusion_feat_col)
        fusion_feat_col_gcn = self.gcn_conv1_col(fusion_feat_col, edge_index)
        fusion_feat_col = (1 - self.w_col[0]) * fusion_feat_col + self.w_col[0] * fusion_feat_col_gcn
        fusion_feat_col_gat = self.gat_conv1_col(fusion_feat_col, edge_index)
        fusion_feat_col = (1 - self.w_col[1]) * fusion_feat_col + self.w_col[1] * fusion_feat_col_gat

        fusion_feat_col_gcn = self.gcn_conv2_col(fusion_feat_col, edge_index)
        fusion_feat_col = (1 - self.w_col[2]) * fusion_feat_col + self.w_col[2] * fusion_feat_col_gcn
        fusion_feat_col_gat = self.gat_conv2_col(fusion_feat_col, edge_index)
        fusion_feat_col = (1 - self.w_col[3]) * fusion_feat_col + self.w_col[3] * fusion_feat_col_gat
        fusion_feat_col = self.gnn_post_col(fusion_feat_col)
