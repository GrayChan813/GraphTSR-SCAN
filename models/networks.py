...
class Cell_Lloc_Pre(nn.Module):
    def __init__(self, num_node_feature, vocab_size, num_text_features, num_classes,
        ...
        self.gcn_conv1_bbox = Sequential('x, edge_index',
                                    [(GCNConv(512, 512 * 8), 'x, edge_index -> x'), norm.LayerNorm(512 * 8), nn.PReLU(), ])
        self.gcn_conv2_bbox = Sequential('x, edge_index',
                                    [(GCNConv(512 * 8, 512), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), ])
        self.gat_conv1_bbox = Sequential('x, edge_index',
                                    [(GATv2Conv(512, 512, heads=8), 'x, edge_index -> x'), norm.LayerNorm(512 * 8), nn.PReLU(), ])
        self.gat_conv2_bbox = Sequential('x, edge_index',
                                    [(GATv2Conv(512 * 8, 512, heads=1), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), ])
        ...
        self.gcn_conv1_row = Sequential('x, edge_index',
                                    [(GCNConv(256 * 3, 512 * 8), 'x, edge_index -> x'), norm.LayerNorm(512 * 8), nn.PReLU(), ])
        self.gcn_conv2_row = Sequential('x, edge_index',
                                    [(GCNConv(512 * 8, 512), 'x, edge_index -> x'), norm.LayerNorm(512), nn.PReLU(), ])
        self.gat_conv1_row = Sequential('x, edge_index',
                                    [(GATv2Conv(256 * 3, 512, heads=8), 'x, edge_index -> x'), norm.LayerNorm(512 * 8), nn.PReLU(), ])
        ...
        self.fc_row_cls = nn.Sequential(*[nn.Linear(512 * 2, 512), nn.PReLU(),
                                          nn.Linear(512, 256), nn.PReLU(),
                                          nn.Linear(256, num_classes), nn.PReLU()])
        self.fc_col_cls = nn.Sequential(*[nn.Linear(512 * 2, 512), nn.PReLU(),
                                          nn.Linear(512, 256), nn.PReLU(),
                                          nn.Linear(256, num_classes), nn.PReLU()])
        ...
        self.w_bbox = nn.Parameter(torch.Tensor([0.5]))
        self.w_img = nn.Parameter(torch.Tensor([0.5]))
        self.w_text = nn.Parameter(torch.Tensor([0.5]))
        self.w_row = nn.Parameter(torch.Tensor([0.5]))
        self.w_col = nn.Parameter(torch.Tensor([0.5]))
    ...
    def forward(self, x, img, imgpos, text, edge_index, edge_pairing_index, phase):
        ...
        box_feat_gcn = self.gcn_conv2_bbox(box_feat_gcn, edge_index)
        box_feat_gat = self.gat_conv1_bbox(box_feat, edge_index)
        box_feat_gat = self.gat_conv2_bbox(box_feat_gat, edge_index)
        box_feat = self.w_bbox * box_feat_gcn + (1 - self.w_bbox) * box_feat_gat
        ...
        img_feat_gcn = self.gcn_conv2_img(img_feat_gcn, edge_index)
        img_feat_gat = self.gat_conv1_img(img_feat, edge_index)
        img_feat_gat = self.gat_conv2_img(img_feat_gat, edge_index)
        img_feat = self.w_img * img_feat_gcn + (1 - self.w_img) * img_feat_gat
        ...
        text_feat_gcn = self.gcn_conv2_text(text_feat_gcn, edge_index)
        text_feat_gat = self.gat_conv1_text(text_feat, edge_index)
        text_feat_gat = self.gat_conv2_text(text_feat_gat, edge_index)
        text_feat = self.w_text * text_feat_gcn + (1 - self.w_text) * text_feat_gat
        ...
        fusion_feat_row_gcn = self.gcn_conv2_row(fusion_feat_row_gcn, edge_index)
        fusion_feat_row_gat = self.gat_conv1_row(fusion_feat_row, edge_index)
        fusion_feat_row_gat = self.gat_conv2_row(fusion_feat_row_gat, edge_index)
        fusion_feat_row = self.w_row * fusion_feat_row_gcn + (1 - self.w_row) * fusion_feat_row_gat

        fusion_feat_col = torch.cat((box_feat_col, img_feat_col, text_feat_col), 1)
        fusion_feat_col_gcn = self.gcn_conv1_col(fusion_feat_col, edge_index)
        fusion_feat_col_gcn = self.gcn_conv2_col(fusion_feat_col_gcn, edge_index)
        fusion_feat_col_gat = self.gat_conv1_col(fusion_feat_col, edge_index)
        fusion_feat_col_gat = self.gat_conv2_col(fusion_feat_col_gat, edge_index)
        fusion_feat_col = self.w_col * fusion_feat_col_gcn + (1 - self.w_col) * fusion_feat_col_gat
